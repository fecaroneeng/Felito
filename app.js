...
function showModal(backEl){
  ...
  [modalBack, modalClientBack, modalFidelityBack, modalSummaryBack, modalClientsBack, document.getElementById('modalFinanceiroBack')].forEach(el=>{ if(el && el !== backEl) el.style.zIndex = 50; });
...
function openStatusPicker(docid, currentStatus, anchorEl, onSelect){
  ...
  STATUS_OPTIONS.forEach(s => {
    ...
    btn.addEventListener('click', async (ev) => {
      ev.stopPropagation();
      popup.remove();
      if(onSelect) onSelect(s); else await quickUpdateStatus(docid, s);
    });
...
/* ---- Toast padrão (usado em cobrança, ajustes em massa, etc.) ---- */
...
/* ---- Chave de cliente (funciona mesmo para clientes sem cadastro) ---- */
function clientKeyOf(it){ return it.clientId || (findClientByNameExact(it.cliente)||{}).id || ('nome::' + String(it.cliente||'—').trim()); }
function clientKeyName(key){ return String(key).indexOf('nome::') === 0 ? String(key).slice(6) : ((findClientById(key)||{}).name || '—'); }
function clientKeyPhone(key){
  if(String(key).indexOf('nome::') === 0){ const it = items.find(x => clientKeyOf(x) === key && x.telefone); return it ? it.telefone : ''; }
  return (findClientById(key)||{}).phone || '';
}
/* status "fechado": não entra mais em cobrança */
function isClosedStatus(s){ return s === 'Pago' || s === 'Cancelado' || isResgateStatus(s); }
...
function renderTable(){
  ...
  view.forEach((p)=>{
    ...
    const btnConta = document.createElement('button'); btnConta.type = 'button'; btnConta.className = 'icon-btn icon-btn-conta'; btnConta.title = 'Copiar conta do cliente (cobrança)'; btnConta.textContent = '💰';
    btnConta.setAttribute('data-action','copy-account'); btnConta.setAttribute('data-docid', String(docid));

    actionsTd.appendChild(btnConta); actionsTd.appendChild(btnEdit); actionsTd.appendChild(btnEditNum); actionsTd.appendChild(btnDel);

    // Adiciona listeners diretamente
    btnConta.addEventListener('click', (ev)=>{ ev.stopPropagation(); ev.preventDefault(); copyClientAccount(p.cliente, p.clientId); });
...
function renderClientsTable(query = ''){
  ...
  items.forEach(it=>{
    ...
    if(!itClientId || receivablesMap[itClientId] === undefined) return;
    if(isClosedStatus(it.status)) return;
  ...
  // lista vertical: uma linha por cliente
  if(!list.length){
  ...
  if(sortSelectEl2) sortSelectEl2.addEventListener('change', ()=> renderClientsTable(query));
...
/* Monta a mensagem de cobrança para uma "chave" de cliente (clientId ou 'nome::Fulano') */
function buildAccountMessage(key){
  const map = {}; let total = 0;
  items.forEach(it=>{
    if(clientKeyOf(it) !== key) return;
    if(isClosedStatus(it.status)) return;
    const v = Number(it.qtd||0) * Number(it.valor||0);
    if(v <= 0) return;
  ...
  let msg = '';
  Object.keys(map).forEach(k=>{ const l = map[k]; msg += `${formatBRL(l.total)} por ${l.qtd} ${l.sabor} ${l.tam}\n`; total += l.total; });
  if(!msg) return null;
  msg += `\nTotal ${formatBRL(total)}\nPix CNPJ: 53643402000170`;
  return msg;
}
/* alias por compatibilidade com o clientId direto */
function buildPendingMessageForClient(clientId){ return buildAccountMessage(clientId); }

/* Copia a conta de um cliente a partir de um item de pedido (usado pelo botão 💰 na tabela) */
function copyClientAccount(cliente, clientId){
  const key = clientId || (findClientByNameExact(cliente||'')||{}).id || ('nome::' + String(cliente||'—').trim());
  const msg = buildAccountMessage(key);
  if(!msg){ showToast(`<b>${clientKeyName(key)}</b> não tem valores em aberto.`); return; }
  copyTextToClipboard(msg)
    .then(()=> showToast(`📋 Conta de <b>${clientKeyName(key)}</b> copiada<br><span style="font-weight:400;font-size:.78rem">Cole no WhatsApp para cobrar</span>`))
...
/* ============================================================
   ...
   ============================================================ */
const modalFinanceiroBack = document.getElementById('modalFinanceiroBack');
const financeiroContent = document.getElementById('financeiroContent');
const financeiroSearch = document.getElementById('financeiroSearch');
const financeiroClose = document.getElementById('financeiroClose');
const btnFinanceiro = document.getElementById('btnFinanceiro');
const finTabsEl = document.getElementById('finTabs');

let finTab = 'receber'; // 'receber' | 'entregar' | 'todas'
let finExpanded = {};
let finSearchTerm = '';

function finTabMatch(it){
  const isReceber = !isClosedStatus(it.status) && (Number(it.qtd||0)*Number(it.valor||0)) > 0;
  const isEntregar = it.status === 'A Fazer' || it.status === 'À Entregar';
  if(finTab === 'receber') return isReceber;
  if(finTab === 'entregar') return isEntregar;
  return isReceber || isEntregar;
}
...
function finBuildGroups(){
  ...
  items.forEach(it=>{
    ...
    const key = clientKeyOf(it);
    if(!groups[key]) groups[key] = { key, name: clientKeyName(key), phone: clientKeyPhone(key), list: [], receber: 0, entregar: 0 };
    groups[key].list.push(it);
    if(!isClosedStatus(it.status) && (Number(it.qtd||0)*Number(it.valor||0)) > 0) groups[key].receber += Number(it.qtd||0)*Number(it.valor||0);
    if(it.status === 'A Fazer' || it.status === 'À Entregar') groups[key].entregar += 1;
  });
...
async function finBulkUpdate(key, newStatus){
  const targets = items.filter(it => clientKeyOf(it) === key && finTabMatch(it));
  if(!targets.length) return;
  targets.forEach(it=>{ it.status = newStatus; if(isResgateStatus(newStatus)) it.valor = 0; });
  saveAllLocal();
  renderTable();
  showToast(`✅ ${targets.length} item(ns) de <b>${clientKeyName(key)}</b> → ${newStatus}`);
  if(window.auth && auth.currentUser && window.db){
...
function renderFinanceiro(){
  ...
  if(!groups.length){
    financeiroContent.innerHTML = html + `<div class="fin-empty">Tudo em dia por aqui. Nenhuma pendência nesta visão.</div>`;
    return;
  }
...
function openFinanceiro(){
  finSearchTerm = '';
  if(financeiroSearch) financeiroSearch.value = '';
  renderFinanceiro();
  showModal(modalFinanceiroBack);
}
function refreshFinanceiroIfOpen(){
  if(!modalFinanceiroBack || modalFinanceiroBack.style.display !== 'flex') return;
  renderFinanceiro();
}

if(btnFinanceiro) btnFinanceiro.addEventListener('click', openFinanceiro);
if(financeiroClose) financeiroClose.addEventListener('click', ()=> hideModal(modalFinanceiroBack));
if(financeiroSearch) financeiroSearch.addEventListener('input', ()=>{ finSearchTerm = financeiroSearch.value; renderFinanceiro(); });

if(finTabsEl) finTabsEl.addEventListener('click', ev=>{
  const btn = ev.target.closest('[data-ftab]');
  if(!btn) return;
  finTabsEl.querySelectorAll('.sv2-tab').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  finTab = btn.getAttribute('data-ftab');
  renderFinanceiro();
});

if(financeiroContent) financeiroContent.addEventListener('click', async ev=>{
  ...
  if(act === 'fin-toggle'){ finExpanded[key] = !finExpanded[key]; renderFinanceiro(); return; }
  ...
  if(act === 'fin-baixa'){
    const n = items.filter(it => clientKeyOf(it) === key && finTabMatch(it)).length;
    if(!confirm(`Marcar como PAGO os ${n} item(ns) listados de ${clientKeyName(key)}?`)) return;
    finBulkUpdate(key, 'Pago');
    return;
  }

  if(act === 'fin-bulk'){ openStatusPicker(null, null, el, s => finBulkUpdate(key, s)); return; }

  if(act === 'fin-status'){
    const idx = findItemIndexByDocId(docid);
    if(idx < 0) return;
    openStatusPicker(docid, items[idx].status, el, s => quickUpdateStatus(docid, s));
    return;
  }

  if(act === 'fin-edit'){ openEditModal(docid); return; }
});

if(modalFinanceiroBack) modalFinanceiroBack.addEventListener('click', ev=>{
  if(ev.target === modalFinanceiroBack && modalFinanceiroBack.dataset.justOpened !== '1') hideModal(modalFinanceiroBack);
});
  ...
  <div class="sv2-entrada-form" style="margin-bottom:14px;border:2px solid var(--yellow);border-radius:12px;padding:16px;background:#fffdf7">
    ...
    <div id="sv2CartSection" style="margin-top:12px;border-top:1px solid #e0d9cf;padding-top:10px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
        <strong style="font-size:0.9rem">Produtos do lote (${sv2EntranceCart.length})</strong>
